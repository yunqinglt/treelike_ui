...
/* Private callback identity for containers created by the raw tree builder. */
static void draw_group_container(UiBuffer *buffer, void *context)
{
    (void)buffer;
    (void)context;
}
                                ...
                                const UiRenderTreeDebugOptions *debug)
{
    ...
    for (size_t group = 0; group < count; ++group) {
        ...
        if (group_sizes[group] > 1) {
            ...
            container->draw = draw_group_container;
        }
...
static void draw_group_debug_bounds(UiBuffer *buffer, pixel_t color)
{
    ...
    if (buffer->draw != draw_group_container || buffer->parent == NULL ||
...
{
    const UiRoundedRectStyle fallback = {8, COLOR_RED, COLOR_BLACK, true};
