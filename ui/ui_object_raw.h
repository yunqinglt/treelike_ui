/**
 * @file ui_object_raw.h
 * Low-level UiBuffer draw callbacks and the demonstration proximity tree.
 *
 * This layer is intended for callers which draw directly into a UiBuffer and
 * keep widget-specific state in the callback context. It is deliberately
 * separate from the retained, high-level UiObject model.
 */

#ifndef TREELIKE_UI_UI_OBJECT_RAW_H
#define TREELIKE_UI_UI_OBJECT_RAW_H
...
/**
 * Ephemeral description of a successfully built raw render tree.
 *
...
/** Build a temporary proximity tree from low-level draw callbacks. */
bool ui_build_render_tree(UiBuffer *root, const UiControl *controls,
                          size_t count, int grouping_threshold);

/** Build a proximity tree and optionally observe its completed topology. */
